...
数据库中存在ID 1~100 的数据，我们并发查询数据库中不存在的记录ID 101~110
...
- 1、同时设置ID 1~100 的缓存
- 2、查询1~100的数据，命中缓存
- 3、手动删除ID 1~10 的缓存，模拟缓存失效
- 4、100并发请求ID 1~10 的数据30次
...
并发查询数据库中不存在的ID 101~110 的数据，可以看出每个ID都只执行了一次数据库查询并设置缓存，之后请求都命中了缓存，有效防止了缓存穿透问题，以下为日志片段
...
并发请求失效缓存ID为 1~ 10 的记录，可以看到每个ID都只查询了一次数据库并设置缓存，之后的请求都命中了缓存
